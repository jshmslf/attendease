...
import { LogOut, User, Camera, Users, LayoutDashboard, Loader2, Clock } from "lucide-react";
      ...
      <header className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center bg-white dark:bg-zinc-900/50 backdrop-blur-md sticky top-0 z-50">
        ...
        <div className="flex items-center gap-4">
          ...
          {user ? (
            ...
            <Link href="/login" className="px-4 py-2 bg-[var(--accent-color)] text-white rounded-[var(--radius)] text-sm font-semibold hover:opacity-90 transition-all">
              Sign In
            </Link>
          )}
        </div>
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 max-w-5xl mx-auto w-full">
        {!user ? (
          <div className="text-center py-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <Link
              href="/login"
              className="px-8 py-4 bg-[var(--accent-color)] text-white rounded-[var(--radius)] text-lg font-bold hover:opacity-90 transition-all shadow-xl shadow-[var(--accent-color)]/20"
            >
              Get Started
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full animate-in fade-in duration-500">
            ...
            {(user.role === "ADMIN") && (
              <Link
                href="/scan"
                className="group flex flex-col p-8 bg-white dark:bg-zinc-900 rounded-[calc(var(--radius)+4px)] border border-zinc-200 dark:border-zinc-800 hover:border-[var(--accent-color)] transition-all hover:shadow-xl shadow-sm"
              >
                <div className="w-14 h-14 bg-[var(--accent-color)]/10 rounded-[var(--radius)] flex items-center justify-center mb-6 text-[var(--accent-color)] group-hover:scale-110 transition-transform">
                  <Camera className="w-7 h-7" />
                </div>
                <h2 className="text-2xl font-bold mb-2 text-zinc-900 dark:text-zinc-50 group-hover:text-[var(--accent-color)] transition-colors">Face Scanning</h2>
                <p className="text-zinc-500 dark:text-zinc-400 text-sm">Automated attendance scanning station for classroom or gate entry.</p>
                <div className="mt-auto pt-6 flex items-center text-[10px] font-bold text-[var(--accent-color)] uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">
                  Open Scanner →
                </div>
              </Link>
            )}
